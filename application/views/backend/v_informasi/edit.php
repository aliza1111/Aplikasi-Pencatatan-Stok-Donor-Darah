...
<head>
  <meta charset="utf-8">
  <title>Edit Informasi Detail Darah</title>
  ...
  <div class="container-fluid position-relative d-flex p-0">
    ...
    <div class="sidebar pe-4 pb-3">
      <nav class="navbar bg-secondary navbar-dark">
        <!-- logo -->
        <a href="<?php echo site_url('Home') ?>" class="navbar-brand mx-4 mb-3">
    ...
    <div class="content">
      ...
      <nav class="navbar navbar-expand bg-secondary navbar-dark sticky-top px-4 py-0">
        <a href="<?php echo site_url('Home') ?>" class="navbar-brand d-flex d-lg-none me-4">
          <h2 class="text-primary mb-0"><img src="<?php echo base_url('assets/img/logo.png'); ?>" width="70"><i></i></h2>
        </a>
      ...
      <div class="container-fluid pt-4 px-4">
        <div class="bg-secondary rounded h-100 p-4">
          <h6 class="mb-4">Edit Data Informasi Detail Darah</h6>
          <form action="<?php echo site_url('Informasi/update/' . $edit['id_darah']) ?>" method="post">
            <div class="mb-3">
              <label class="form-label">Id Darah</label>
              <input type="text" class="form-control" name="id_darah" disabled value="<?php echo $edit['id_darah'] ?>">
              <input type="hidden" name="id_darah" value="<?php echo $edit['id_darah'] ?>">
            </div>
            <div class="mb-3">
              ...
              <select name="id_stok" class="form-control" required>
                ...
                <?php foreach ($stok_darah->result_array() as $r) : ?>
                  <option value="<?php echo $r['id_stok'] ?>"><?php echo $r['darah']; ?></option>
                <?php endforeach; ?>
              </select>
              <p></p>
            </div>
            <div class="mb-3">
              <label class="form-label">Goldar</label>
              <select class="form-control" name="goldar">
                <option value="">Pilih Goldar</option>
                <option value="A" <?php if ($edit['goldar'] == 'A') echo 'selected'; ?>>A</option>
                <option value="B" <?php if ($edit['goldar'] == 'B') echo 'selected'; ?>>B</option>
                <option value="AB" <?php if ($edit['goldar'] == 'AB') echo 'selected'; ?>>AB</option>
                <option value="O" <?php if ($edit['goldar'] == 'O') echo 'selected'; ?>>O</option>
              </select>
            </div>
            <div class="mb-3">
              ...
              <select class="form-control" name="rhesus">
                ...
                <option value="+" <?php if ($edit['rhesus'] == '+') echo 'selected'; ?>>+</option>
                <option value="-" <?php if ($edit['rhesus'] == '-') echo 'selected'; ?>>-</option>
              </select>
            </div>
            <div class="mb-3">
              ...
              <select class="form-control" name="produk">
                ...
                <option value="WE" <?php if ($edit['produk'] == 'WE') echo 'selected'; ?>>WE</option>
                <option value="TCA" <?php if ($edit['produk'] == 'TCA') echo 'selected'; ?>>TCA</option>
                <option value="TC" <?php if ($edit['produk'] == 'TC') echo 'selected'; ?>>TC</option>
                <option value="PRC" <?php if ($edit['produk'] == 'PRC') echo 'selected'; ?>>PRC</option>
                <option value="FFP" <?php if ($edit['produk'] == 'FFP') echo 'selected'; ?>>FFP</option>
                <option value="BC" <?php if ($edit['produk'] == 'BC') echo 'selected'; ?>>BC</option>
                <option value="AHF" <?php if ($edit['produk'] == 'AHF') echo 'selected'; ?>>AHF</option>
              </select>
            </div>
            <div class="d-flex">
              <button type="submit" name="button" class="btn btn-success me-2">Perbarui</button>
              <a href="<?php echo site_url('Informasi') ?>" class="btn btn-primary">Batal</a>
            </div>
          </form>
        </div>
      </div>
